// em72.js  –  works with SerialPort v12.x or v14.x
...
/* -------- configuration ------------------------------------------------- */
const PORT = process.argv[2] || 'COM7';   // eg. '/dev/ttyUSB0' on Linux/Mac
const BAUD = 2400;
/* ------------------------------------------------------------------------ */
...
/* ---------------------------------------------------------------------- */
/*  Decoder that covers *all* EM‑72 modes seen so far                     */
/* ---------------------------------------------------------------------- */
...
/* ------------------------------------------------------------------------ */
function handleFrame(pkt) {
  /* ---- numeric field ----------------------------------------------- */
  let digitStr = '';
  for (let i = 1; i <= 10; ++i) {
    if (pkt[i] >= 0x30 && pkt[i] <= 0x39) digitStr += String.fromCharCode(pkt[i]);
  }
  const digits = parseInt(digitStr || '0', 10);
  ...
  let mode = 'unknown', unit = '';
  //console.log(funcByte);
  if (funcByte === 0x04) { mode = 'resistance';        unit = 'Ω';  }
  else if (funcByte === 0x10) { mode = 'capacitance';  unit = 'F';  }
  else if (unitByte === 0x78) { mode = 'diode';        unit = 'V';  }
  else if (unitByte === 0xE8) {
    mode = (rangeByte === 0x40) ? 'voltage‑ac' : 'voltage‑dc';
    unit = 'V';
  }

  /* ---- decimal placement ------------------------------------------- */
  let shift = { 0x00: 3, 0x40: 1, 0x80: 3 }[rangeByte & 0xF0] ?? 3;
  if (mode === 'diode') shift = 4;           // diode uses 10 mV resolution

  const value = digits / 10 ** shift;
  const negative = pkt[1] === 0x2D;          // byte 1 = '-' ?

  /* ---- flags -------------------------------------------------------- */
  const hold = !!(pkt[17] & 0x01);           // HOLD annunciator
  ...
  const sample = {
    ...
    hold,
    raw  : bytesToHex(pkt.slice(0, 26)),     // optional debug
  };
  console.log(sample);
}
